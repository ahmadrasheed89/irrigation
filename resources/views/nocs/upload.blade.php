...
<div class="content">
        ...
        <tbody>
            ...
            @endforeach --}}
            @forelse ($allNocCategories as $category)
                @php
                    // Check if there's a tender for this category under this scheme
                    $nocFile = $nocFiles->firstWhere('noc_category_id', $category->id);
                @endphp
            <tr>
                <td>{{ $category->name }}</td>
                ...
                <td>{{ $nocFile ? $nocFile->date->format('Y-m-d') : '-' }}</td>
                {{-- <td>
                ...
                @if ($nocFile)
                    <td>
                        <ul>
                            @foreach($nocFile->attached_files ?? [] as $file)
                                <a href="{{ asset('storage/'.$file) }}" target="_blank" class="btn btn-success">view</a>
                            @endforeach
                        </ul>
                    </td>
                    <td>
                         <form action="{{ route('nocs.uploaddestroy', $nocFile) }}" method="POST" class="d-inline">
                            @csrf @method('DELETE')
...
<div class="modal fade" id="uploadModal" tabindex="-1" aria-labelledby="uploadModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form {{--id="uploadForm" --}} action="{{ route('nocs.uploadfile') }}" method="POST" enctype="multipart/form-data">
      <div class="modal-content">
        ...
        <div class="modal-body">
            @csrf
            <input type="hidden" name="noc_category_id" id="noc_category_id">
...
$(function() {
    ...
    $('.uploadBtn').click(function() {
        ...
        let nocId = $(this).data('noc-id');
        $('#noc_category_id').val(categoryId);
        $('#noc_id').val(nocId);
        $('#uploadAlert').html('');
        $('#file').val('');
        uploadModal.show();
